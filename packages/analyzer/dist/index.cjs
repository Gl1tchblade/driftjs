...
__export(index_exports, {
  BaseORMDetector: () => BaseORMDetector,
  ConnectionPool: () => ConnectionPool,
  DatabaseAnalyzer: () => DatabaseAnalyzer,
  DrizzleDetector: () => DrizzleDetector,
  MySQLConnection: () => MySQLConnection,
  PostgreSQLAdapter: () => PostgreSQLAdapter,
  PostgreSQLConnection: () => PostgreSQLConnection,
  PrismaDetector: () => PrismaDetector,
  SQLiteConnection: () => SQLiteConnection,
  TypeORMDetector: () => TypeORMDetector,
...
var import_core = require("@driftjs/core");
var BaseORMDetector = class {
  ...
  async checkPackageJsonDependencies(projectPath, dependencies) {
    ...
    const packageResult = await (0, import_core.readJsonFile)(packageJsonPath);
  ...
  async checkFiles(projectPath, filePaths) {
    ...
    for (const filePath of filePaths) {
      const fullPath = (0, import_path.join)(projectPath, filePath);
      const fileExists = await (0, import_core.exists)(fullPath);
      if (fileExists) {
        existing.push(await (0, import_core.createFilePath)(filePath, projectPath));
      } else {
  ...
  async findFilesByPattern(projectPath, patterns, directories = ["."]) {
    ...
    for (const directory of directories) {
      const fullDirectory = (0, import_path.join)(projectPath, directory);
      for (const pattern of patterns) {
        const files = await (0, import_core.findFiles)(fullDirectory, pattern, true);
        allFiles.push(...files);
      }
    }
    return allFiles;
...
var import_core2 = require("@driftjs/core");
var PrismaDetector = class extends BaseORMDetector {
  ...
  async extractConfig(projectPath) {
    ...
    const migrationDirectory = await (0, import_core2.createFilePath)("prisma/migrations", projectPath);
    const schemaResult = await (0, import_core2.readFileContent)(schemaFile.absolute);
  ...
  async getDatabaseConfig(projectPath) {
    ...
    const schemaResult = await (0, import_core2.readFileContent)(config.schemaFile.absolute);
    if (!schemaResult.success) {
      return null;
    }
